...
function initSlideshow() {
  ...
  const show = (index) => {
    ...
    dots.forEach((dot, i) => dot.classList.toggle("is-active", i === state.slideIndex));
  };
...
async function handleLogin(event) {
  ...
  try {
    const body = await submitLogin({ username, password, remember });
...
async function checkSession() {
  try {
    ...
    // Keep the login page usable if the session endpoint is temporarily unavailable.
  }
}
